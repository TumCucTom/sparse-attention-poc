...
if not results:
    ...
    print(f"\n{'Seq Len':>10} {'Attention':>12} {'Speed':>10} {'Baseline':>10} {'Speedup':>8}")
    print("-"*70)
    ...
    for seq_len in sorted(set(r['seq_len'] for r in results)):
        ...
        for r in sorted([x for x in results if x['seq_len'] == seq_len], key=lambda x: x['attention_type']):
            speed = r['avg_speed_tokens_per_sec']
            speedup = speed / baseline if baseline > 0 else 0
            print(f"  {r['attention_type']:>12} {speed:>10.1f} {baseline:>10.1f} {speedup:>7.2f}x")

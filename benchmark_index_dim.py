#!/usr/bin/env python3
"""Benchmark sparse with reduced index_dim (16 vs 32) on Qwen2.5-14B at 128K."""
import os
import torch
...
# Test with different index_dim values
configs = [
    (32, "baseline"),
    (16, "reduced"),
    (8, "small"),
...
os.makedirs("hpc_results_index_dim", exist_ok=True)

for index_dim, label in configs:
    print(f"\n=== Testing index_dim={index_dim} ({label}) ===")
    ...
    def replace(module):
        if isinstance(module, Qwen2Attention):
            ...
            attn = MiniMaxSparseAttention(
                ...
                block_size=32,  # Best config
                top_k_blocks=4,  # Best config
                index_dim=index_dim,
    ...
    with open(f"hpc_results_index_dim/sparse_idx{index_dim}_128k.json", 'w') as f:
        json.dump(result, f, indent=2)

    print(f"  => index_dim={index_dim}: {avg_speed:.1f} tok/s")

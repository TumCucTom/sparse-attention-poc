#!/usr/bin/env python3
"""Benchmark MiniMax-M2.7 with streaming attention at 128K.

Proper streaming attention replacement that avoids block_scores matrix.
"""
import os
import subprocess
...
import json
import types
from streaming_llm_sparse import replace_attention_with_streaming
...
sink_size = int(os.environ.get('SINK_SIZE', '4'))
...
# Replace attention with streaming
print(f"Replacing attention with streaming (window={window_size}, sink={sink_size})...")
try:
    num_replaced = replace_attention_with_streaming(model, window_size=window_size, sink_size=sink_size)
...
attn_type = "streaming" if replace_success else "dense"
...
with open(f"hpc_results_minimax/streaming_proper_{context_size}.json", 'w') as f:

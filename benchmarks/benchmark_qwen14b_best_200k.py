#!/usr/bin/env python3
"""Benchmark sparse with best config (k=4, b=32) on Qwen2.5-14B at 200K."""
import os
import torch
...
# Replace attention with best config
...
def replace(module):
    if isinstance(module, Qwen2Attention):
        ...
        attn = MiniMaxSparseAttention(
            ...
            block_size=32,  # Best config from sweep
            top_k_blocks=4,  # Best config from sweep
            index_dim=32,
...
print("Benchmarking sparse at 200K with k=4, b=32...")
times = []
for i in range(3):
...
os.makedirs("hpc_results_qwen14b_best", exist_ok=True)
with open("hpc_results_qwen14b_best/sparse_200k_best.json", 'w') as f:
    json.dump(result, f, indent=2)

print(f"\nSparse (k=4, b=32) at 200K: {avg_speed:.1f} tok/s")

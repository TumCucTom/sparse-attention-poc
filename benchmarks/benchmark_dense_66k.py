#!/usr/bin/env python3
"""Benchmark dense attention at ~66K tokens for fair comparison."""
import os
import torch
...
target_tokens = 66666  # Same as sparse test
num_tokens = 32
...
# Create ~66K token prompt (same as sparse test)
base_prompt = "The theory of quantum mechanics describes how particles behave at the atomic level. "
prompt = base_prompt * 40000  # Should give ~66K tokens
prompt = prompt[:target_tokens * 2]
...
os.makedirs("hpc_results_dense_v2", exist_ok=True)
with open("hpc_results_dense_v2/dense_66k.json", 'w') as f:
